...
pub struct OstensibleGeneralizedFilterExec<H: ProverHonestyMarker> {
    pub(super) aliased_results: Vec<AliasedDynProofExpr>,
    pub(super) input: DynProofPlan,
    /// TODO: add docs
    pub(crate) where_clause: DynProofExpr,
    phantom: PhantomData<H>,
}

impl<H: ProverHonestyMarker> OstensibleGeneralizedFilterExec<H> {
...
impl<H: ProverHonestyMarker> ProofPlan for OstensibleGeneralizedFilterExec<H>
where
    OstensibleGeneralizedFilterExec<H>: ProverEvaluate,
{
    ...
    fn verifier_evaluate<S: Scalar>(
        ...
        let (input_evals, input_one_eval) = self.input.verifier_evaluate(builder, accessor, None, one_eval_map)?;
        // 1. selection
        let selection_eval =
            self.where_clause
...
pub type FilterExec = OstensibleGeneralizedFilterExec<HonestProver>;
