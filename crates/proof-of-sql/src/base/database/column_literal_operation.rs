...
// Binary operations on slices and literals of the same type

/// Apply a comparison function to a slice and a literal element-wise.
pub(super) fn slice_literal_cmp<T>(slice: &[T], literal: T, op: fn(T, T) -> bool) -> Vec<bool>
where
    T: PartialEq + Debug,
{
    slice.iter()
...
/// Try to check whether a slice is equal to a literal element-wise.
pub(super) fn slice_literal_eq<T>(slice: &[T], literal: T) -> Vec<bool>
where
    T: PartialEq + Debug,
{
    slice_literal_cmp(slice, literal, |x, y| x == y)
}

/// Try to check whether a slice is less than or equal to a literal element-wise.
pub(super) fn slice_literal_le<T>(slice: &[T], literal: T) -> Vec<bool>
where
    T: PartialOrd + Debug,
{
    slice_literal_cmp(slice, literal, |x, y| x <= y)
...
/// Try to check whether a slice is greater than or equal to a literal element-wise.
pub(super) fn slice_literal_ge<T>(slice: &[T], literal: T) -> Vec<bool>
where
    T: PartialOrd + Debug,
{
    slice_literal_cmp(slice, literal, |x, y| x >= y)
...
/// Try to add a literal to a slice element-wise.
///
/// Note that we do check for integer overflow.
pub(super) fn try_add_slice_to_literal<T>(slice: &[T], literal: T) -> ColumnOperationResult<Vec<T>>
where
    T: CheckedAdd<Output = T> + Debug,
{
    slice
        .iter()
        .map(|x| -> ColumnOperationResult<T> {
            x.checked_add(&literal)
                .ok_or(ColumnOperationError::IntegerOverflow {
                    error: format!("Overflow in integer addition {x:?} + {literal:?}"),
                })
        })
        .collect::<ColumnOperationResult<Vec<T>>>()
}

/// Try to subtract a literal from a slice element-wise.
///
/// Note that we do check for integer overflow.
pub(super) fn try_subtract_literal_from_slice<T>(slice: &[T], literal: T) -> ColumnOperationResult<Vec<T>>
where
    T: CheckedSub<Output = T> + Debug,
{
    slice
        .iter()
        .map(|x| -> ColumnOperationResult<T> {
            x.checked_sub(&literal)
                .ok_or(ColumnOperationError::IntegerOverflow {
                    error: format!("Overflow in integer subtraction {x:?} - {literal:?}"),
                })
        })
        .collect::<ColumnOperationResult<Vec<T>>>()
}

/// Try to subtract a slice from a literal element-wise.
///
/// Note that we do check for integer overflow.
pub(super) fn try_subtract_slice_from_literal<T>(slice: &[T], literal: T) -> ColumnOperationResult<Vec<T>>
where
    T: CheckedSub<Output = T> + Debug,
{
    slice
        .iter()
        .map(|x| -> ColumnOperationResult<T> {
            literal.checked_sub(x)
                .ok_or(ColumnOperationError::IntegerOverflow {
                    error: format!("Overflow in integer subtraction {literal:?} - {x:?}"),
                })
        })
        .collect::<ColumnOperationResult<Vec<T>>>()
}

/// Try to multiply a slice by a literal element-wise.
///
/// Note that we do check for integer overflow.
pub(super) fn try_multiply_slice_by_literal<T>(slice: &[T], literal: T) -> ColumnOperationResult<Vec<T>>
where
    T: CheckedMul<Output = T> + Debug,
{
    slice
        .iter()
        .map(|x| -> ColumnOperationResult<T> {
            x.checked_mul(&literal)
                .ok_or(ColumnOperationError::IntegerOverflow {
                    error: format!("Overflow in integer multiplication {x:?} * {literal:?}"),
                })
        })
        .collect::<ColumnOperationResult<Vec<T>>>()
}

/// Try to divide a literal by a slice element-wise.
///
/// Note that we do check for division by 0.
#[allow(dead_code)]
pub(super) fn try_divide_literal_by_slice<T>(slice: &[T], literal: T) -> ColumnOperationResult<Vec<T>>
where
    T: CheckedDiv<Output = T> + Debug,
{
    slice
        .iter()
        .map(|x| -> ColumnOperationResult<T> {
            x.checked_div(&literal).ok_or(ColumnOperationError::DivisionByZero)
        })
        .collect::<ColumnOperationResult<Vec<T>>>()
}

/// Try to divide a slice by a literal element-wise.
///
/// Note that we do check for division by 0.
#[allow(dead_code)]
pub(super) fn try_divide_slice_by_literal<T>(slice: &[T], literal: T) -> ColumnOperationResult<Vec<T>>
where
    T: CheckedDiv<Output = T> + Debug,
{
    slice
        .iter()
        .map(|x| -> ColumnOperationResult<T> {
            literal.checked_div(x).ok_or(ColumnOperationError::DivisionByZero)
        })
        .collect::<ColumnOperationResult<Vec<T>>>()
}
...
/// Check whether a slice is equal to a literal element-wise, casting the slice to the type of the literal.
pub(super) fn slice_eq_cast_slice<SmallerType, LargerType>(slice: &[SmallerType], literal: LargerType) -> Vec<bool>
where
    SmallerType: Copy + Debug + Into<LargerType>,
    LargerType: PartialEq + Copy + Debug,
{
    slice.iter()
        .map(|x| -> bool { Into::<LargerType>::into(*x) == literal })
        .collect::<Vec<_>>()
}
